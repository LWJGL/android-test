...
final class HelloVulkan {
	...
	private static final ByteBuffer KHR_surface = memASCII(VK_KHR_SURFACE_EXTENSION_NAME);
	private static final ByteBuffer KHR_android_surface = memASCII(VK_KHR_ANDROID_SURFACE_EXTENSION_NAME);
	...
	HelloVulkan(long nativeWindow) {
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkAndroidSurfaceCreateInfoKHR createInfo = VkAndroidSurfaceCreateInfoKHR.mallocStack(stack)
				.sType(VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR)
				.pNext(NULL)
				.flags(0)
				.window(nativeWindow);
			check(vkCreateAndroidSurfaceKHR(inst, createInfo, null, lp));
			surface = lp.get(0);
		}
	}
	...
	private static void check(int errcode) {
		if (errcode != 0)
			throw new IllegalStateException(String.format("Vulkan error [%d]", errcode));
	}
	...
	private void demo_init_vk() {
		...
		try (MemoryStack stack = stackPush()) {
			...
			if (VALIDATE) {
				...
				if (ip.get(0) > 0) {
					...
					if (!validation_found) {
						...
						for (int i = 0; i < instance_validation_layers_alt2.length; i++) {
			...
			extension_names.put(KHR_surface);
			extension_names.put(KHR_android_surface);
			...
			VkApplicationInfo app = VkApplicationInfo.callocStack(stack)
			...
			VkInstanceCreateInfo inst_info = VkInstanceCreateInfo.callocStack(stack)
			...
			if (VALIDATE) {
				dbgCreateInfo = VkDebugReportCallbackCreateInfoEXT.callocStack(stack)
	...
	private boolean demo_draw() {
		try (MemoryStack stack = stackPush()) {
			...
			if (err == VK_ERROR_OUT_OF_DATE_KHR) {
				...
				//System.err.println("vkAcquireNextImageKHR failed: " + err);
				return true;
				//check(err);
			}
	...
	private void demo_destroy_context() {
		for (int i = 0; i < swapchainImageCount; i++) {
